...
import static Client.Controller.ClientController.exitButton;
...
public class ThankYouPage extends JFrame {
    ...
    public ThankYouPage() {
        ...
        ImageIcon checkImage = new ImageIcon("src/src/Res/check.png");
        ...
        // id code
        String eventID = ClientController.generateEventID();
        idCode = new JLabel(eventID);
        ...
        // exit button
        exiButton = new JButton("Register New!");
        exiButton.setBounds(653, 551,185, 56);
        exiButton.setFocusable(false);
        exiButton.setFont(new Font("DM Sans", Font.BOLD, 15));
        exiButton.setForeground(Color.BLUE);
        exiButton.setBorder(BorderFactory.createLineBorder(Color.BLUE, 3, true));
        exiButton.setBackground(Color.WHITE);
        exiButton.addActionListener(e -> {
            exitButton(this.frame );
