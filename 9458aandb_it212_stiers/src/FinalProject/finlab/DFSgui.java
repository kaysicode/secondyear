...
public class DFSgui extends JPanel {
    ...
    private final Stack<Character> dfsStack = new Stack<>();
    private final int radius = 30; // Node circle radius
    private JFrame frame;
    private String traversalResult = ""; // String to store traversal result

    public DFSgui() {
        frame = new JFrame("DFS Animation");
        frame.add(this);
        ...
        if (input != null && input.length() == 1 && graph.containsKey(input.charAt(0))) {
            ...
            dfsStack.push('a'); // Default to 'a' if no valid input
        }

        // Start DFS Animation using javax.swing.Timer
        new Timer(1000, e -> {
            if (!dfsStack.isEmpty()) {
                char current = dfsStack.pop();
                visited.add(current);
                traversalResult += current + " -> "; // Add current node to the result

                // Add neighbors to the stack (simulate recursive DFS)
    ...
    // Method to set the starting vertex for DFS traversal
    public void setStartVertex(char startVertex) {
        if (graph.containsKey(startVertex)) {
            visited.clear(); // Clear previous visits
            dfsStack.clear(); // Clear previous traversal
            dfsStack.push(startVertex); // Set new start vertex
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawString("DFS Path: " + traversalResult, 50, getHeight() - 30);
    }
}
